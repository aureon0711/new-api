...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	} else if usage.OutputTokens == 0 && !common.ChargeForEmptyOutputEnabled {
		// 如果输出为空且未启用空输出计费，则不计费
		quota = 0
		logContent += fmt.Sprintf("（输出为空，未计费）")
		logger.LogInfo(ctx, fmt.Sprintf("output tokens is 0, skip charging, userId %d, channelId %d, "+
...
func PostAudioConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
	...
	} else if usage.CompletionTokens == 0 && !common.ChargeForEmptyOutputEnabled {
		// 如果输出为空且未启用空输出计费，则不计费
		quota = 0
		logContent += fmt.Sprintf("（输出为空，未计费）")
		logger.LogInfo(ctx, fmt.Sprintf("completion tokens is 0, skip charging, userId %d, channelId %d, "+
			"tokenId %d, model %s", relayInfo.UserId, relayInfo.ChannelId, relayInfo.TokenId, relayInfo.OriginModelName))
	} else {
		model.UpdateUserUsedQuotaAndRequestCount(relayInfo.UserId, quota)
		model.UpdateChannelUsedQuota(relayInfo.ChannelId, quota)
	}
